...
class DStar:
    def __init__(self, grid, dynamic_grid, start, goal):
        ...
        # the actual grid map (with dynamic obstacles)
        self.dynamic_grid = dynamic_grid
        # Create a new grid to store nodes
        size_row = len(grid)
        size_col = len(grid[0])
        self.grid_node = [[None for i in range(
            size_col)] for j in range(size_row)]
        ...
        if k_previous == current.h:
            for neighbor in neighbors:
                if neighbor.tag == "NEW" or (neighbor.parent == current) and (neighbor.h != current.h + self.cost(neighbor, current)) or (neighbor.parent != current) and neighbor.h > current.h + self.cost(neighbor, current):
                    ...
                    self.insert(neighbor, current.h +
                                self.cost(neighbor, current))
    ...
    def draw_path(self, grid, title="Path"):
        ...
        ax.add_patch(Rectangle((start.col-0.5, start.row-0.5), 1,
                     1, edgecolor='k', facecolor='g'))  # start
        ax.add_patch(Rectangle((end.col-0.5, end.row-0.5), 1,
                     1, edgecolor='k', facecolor='r'))  # goal
        # Graph settings
        plt.title(title)
        plt.axis('scaled')
