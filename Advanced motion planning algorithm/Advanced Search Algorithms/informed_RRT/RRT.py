...
class RRT:
    ...
    def get_new_point_in_ellipsoid(self, goal_bias, c_best):
        ...
        # Generate a random point in an ellipsoid
            ...
            x_center = np.array([[((self.start.row+self.goal.row)/2)], [((self.goal.col+self.start.col)/2)]])
            ...
            C = np.array([[math.cos(theta), -1*math.sin(theta)], [math.sin(theta), math.cos(theta)]])
            # Compute diagonal matrix - L
            r1 = c_best/2
            r2 = math.sqrt((c_best**2)-(c_min**2))/2
            L = np.array([[r1, 0], [0, r2]])
            # Cast a sample from a unit ball - x_ball
            x = np.random.randn(2)
            r = np.sqrt(np.sum(x ** 2))
            xball = x / r * np.random.rand() ** (1 / 2)
            x_ball = np.array([[xball[0]], [xball[1]]])
            #x_ball = np.array([[random.random()], [random.random()]])
